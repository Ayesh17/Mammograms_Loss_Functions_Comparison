...
class Evaluation_metrices:
    ...
    def calculate_metrics(self,output, target):
        ...
        # print("target", target[1][1])
            ...
            tp1= torch.sum((converted_output == 1) & (converted_target == 1))
            fp1= torch.sum((converted_output == 1) & (converted_target == 0))
            tn1= torch.sum((converted_output == 0) & (converted_target == 0))
            fn1= torch.sum((converted_output == 0) & (converted_target == 1))

            print('-'*200)
            recall = (tp1) / (tp1 + fn1)
            print("recall", i, recall)
            recall_all += recall
            #print(torch.sum((converted_output == 1) & (converted_target == 1)), torch.sum((converted_output == 1) & (converted_target == 0)), torch.sum((converted_output == 0) & (converted_target == 0)), torch.sum((converted_output == 0) & (converted_target == 1)))
            ...
            accuracy = self.calclate_accuracy(tp, tn, fp, fn)
        ...
        print('-' * 200)
        recall = (tp) / (tp + fn)
        print("recalls:", recall_all, recall)

        return tp, tn, fp, fn


    def calclate_accuracy(self, tp, tn, fp, fn):
