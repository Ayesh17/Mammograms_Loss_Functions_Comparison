...
class Semantic_loss_functions:
    ...
    # def dice_loss (self, tp, tn, fp, fn):
    ...
    def tversky_loss(self, y_pred, y_true):
        return 1 - self.tversky_index(y_pred, y_true)
    ...
    def jacard_similarity(self, y_pred, y_true):
        y_true_f = y_true.view(-1)
        y_pred_f = y_pred.view(-1)
    ...
    def jacard_loss(self, y_pred, y_true):
        return 1 - self.jacard_similarity(y_pred, y_true)

    def ssim_loss(self, y_pred, y_true):
        return 1 - torch.image.ssim(y_pred, y_true, max_val=1)

    def unet3p_hybrid_loss(self, y_pred, y_true):
    ...
    def basnet_hybrid_loss(self, y_pred, y_true):
